...
import { analyzeChatScreenshots, analyzeQuick, analyzeDetailed, QuickAnalysisResult, DetailedAnalysisResult, ExtractionResult, generatePersonalizedSoulTypeDescriptions } from '@/services/geminiService';
...
/**
 * ImageFile type for React Native - represents a picked image.
 * In web this was File; in RN we pass URIs and metadata.
 */
...
export async function processAnalysis(personId: string, imageFiles: ImageFile[]): Promise<string> {
  ...
  if (__DEV__ && personId.startsWith('dev-')) {
 ...
 * Results are stored in AsyncStorage for retrieval.
 */
async function processAnalysisDevMode(personId: string, imageFiles: ImageFile[]): Promise<string> {
  const devAnalysisId = 'dev-analysis-' + Date.now();

  try {
    console.log('[DEV MODE] Starting AI analysis with Gemini 2.0 Flash...');

    // Call real Gemini API - extract URIs from ImageFile objects
    const imageUris = imageFiles.map(f => f.uri);
    const aiResult = await analyzeChatScreenshots(imageUris);
    ...
    await AsyncStorage.setItem('dev_analysis_result_' + devAnalysisId, JSON.stringify(storedResult));
    console.log('[DEV MODE] Analysis stored in AsyncStorage:', devAnalysisId);
...
async function runAIAnalysis(analysisId: string, imageFiles: ImageFile[], personId: string): Promise<void> {
  try {
    console.log('Starting AI analysis with Gemini 2.0 Flash...');

    // Gemini API call - extract URIs from ImageFile objects
    const imageUris = imageFiles.map(f => f.uri);
    const aiResult = await analyzeChatScreenshots(imageUris);
...
export async function getAnalysisResult(analysisId: string): Promise<StoredAnalysisResult | null> {
  // DEV MODE or CONTENT MODE: Check AsyncStorage first
  const isContentMode = analysisId.startsWith('dev-analysis-content-');
  if ((__DEV__ || isContentMode) && analysisId.startsWith('dev-analysis-')) {
    // Try to get real Gemini result from AsyncStorage
    const storedResult = await AsyncStorage.getItem('dev_analysis_result_' + analysisId);
    if (storedResult) {
      console.log('getAnalysisResult: DEV MODE - returning real Gemini result from AsyncStorage for', analysisId);
      return JSON.parse(storedResult) as StoredAnalysisResult;
    }
    // Fall back to mock if no real result found
    console.log('getAnalysisResult: DEV MODE - no AsyncStorage result, returning mock for', analysisId);
  ...
  return {
    ...
    personArchetype: personArchetype ? {
      ...
      tagline: personArchetype.tagline || '',
      ...
      sideProfileImageUrl: personArchetype.side_profile_image_url || '',
    ...
    userArchetype: userArchetype ? {
      ...
      sideProfileImageUrl: userArchetype.side_profile_image_url || '',
...
export async function getAnalysisStatus(analysisId: string): Promise<AnalysisStatus> {
  ...
  if ((__DEV__ || isContentMode) && analysisId.startsWith('dev-analysis-')) {
    const status = await AsyncStorage.getItem('analysis_status_' + analysisId);
 ...
 */
export async function startAnalysis(personId: string, imageFiles: ImageFile[]): Promise<string> {
  ...
  if (__DEV__ && personId.startsWith('dev-')) {
    ...
    await AsyncStorage.setItem('analysis_status_' + devAnalysisId, 'pending');
 ...
 */
async function processTwoPhaseAnalysis(analysisId: string, imageFiles: ImageFile[]): Promise<void> {
  ...
  try {
    ...
    const imageUris = imageFiles.map(f => f.uri);
    const { quick, extraction: ext } = await analyzeQuick(imageUris);
    ...
    await AsyncStorage.setItem('dev_analysis_result_' + analysisId, JSON.stringify(partialResult));
    await AsyncStorage.setItem('analysis_status_' + analysisId, 'quick_ready');
    ...
    await AsyncStorage.setItem('dev_analysis_result_' + analysisId, JSON.stringify(fullResult));
    await AsyncStorage.setItem('analysis_status_' + analysisId, 'completed');
    ...
    await AsyncStorage.setItem('analysis_status_' + analysisId, 'error');
  }
}
