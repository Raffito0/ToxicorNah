...
/**
 * Load a scenario JSON from local bundled assets.
 * TODO: Implement local scenario loading for React Native (bundled JSON or asset require).
 * For now, this throws - use loadScenarioFromSupabase() instead.
 */
export async function loadScenario(name: string): Promise<ContentScenario> {
  // TODO: Replace with local asset loading for React Native
  // In the web app this was: fetch(`/scenarios/${name}.json`)
  // For React Native, scenarios should be bundled or fetched from a remote URL.
  throw new Error(`Local scenario loading not yet implemented for React Native. Use loadScenarioFromSupabase() instead. Requested: "${name}"`);
}
...
/**
 * Inject a content scenario into AsyncStorage and simulate the two-phase loading.
 * Returns the analysis ID that ResultsPage can use.
 */
export async function injectContentScenario(scenario: ContentScenario): Promise<string> {
  ...
  await AsyncStorage.setItem('dev_analysis_result_' + analysisId, JSON.stringify(result));
