...
describe('Article Pipeline E2E (Chain 2)', () => {
  ...
  describe('Test 2.1 - keyword → outline → write integration', () => {
    it('locks keyword, generates outline containing ticker, and writes article to NocoDB', async () => {
      ...
      expect(lockCallArgs[0]).toContain(String(keyword.id)); // URL contains keyword ID
      ...
      expect(writeBody.title_text).toBe(articleToWrite.title); // production maps title → title_text
      expect(writeBody.status).toBe('enriching'); // articles enter NocoDB in enriching state
    });
  });
  ...
  describe('Test 2.3 - freshness check redirects article type', () => {
    it('returns contrarian effectiveArticleType when recent article exists for ticker', async () => {
      ...
      expect(freshness.lastPublished).toBe('2026-02-20T10:00:00Z'); // exact date from fixture
      // NocoDB was queried exactly once, URL contains table and ticker
    ...
    it('returns insider_buying effectiveArticleType when no recent article exists', async () => {
      ...
      expect(freshness.lastPublished).toBeUndefined(); // no recent article = no lastPublished
    });
  });
