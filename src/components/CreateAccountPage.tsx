...
import { migrateGuestToUser, linkStripeCustomer } from '../services/userStateService';
...
export function CreateAccountPage({ stripeSessionId, analysisId, onAccountCreated }: CreateAccountPageProps) {
  ...
  async function handlePostAuth(userId: string) {
    ...
    // Link Stripe payment to user if available
    if (stripeSessionId) {
      try {
        await linkStripeCustomer(userId, stripeSessionId);
      } catch (err) {
  ...
  async function handleAppleSignIn() {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'apple',
        options: {
          redirectTo: `${window.location.origin}/payment-success?session_id=${stripeSessionId || ''}&guest=true&type=subscription&analysis=${analysisId || ''}`
        }
      });
      if (error) throw error;
    } catch (err: any) {
  ...
  function handleCodeInput(index: number, value: string) {
    ...
    if (value && index < 5) {
      const nextInput = document.getElementById(`create-code-input-${index + 1}`);
      nextInput?.focus();
    }
  }
  ...
  function handleCodeKeyDown(index: number, e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === 'Backspace' && !verificationCode[index] && index > 0) {
      const prevInput = document.getElementById(`create-code-input-${index - 1}`);
      prevInput?.focus();
    }
  }
  ...
  return (
    <div
      ...
      style={{
        paddingLeft: '30px',
        paddingRight: '30px',
        filter: showVerification ? 'blur(8px)' : 'none'
      }}
    >
      <motion.div
      ...
      >
        <div className="flex flex-col items-center mb-12">
          ...
          >
            Save your results and unlock everything
          </p>
        </div>
